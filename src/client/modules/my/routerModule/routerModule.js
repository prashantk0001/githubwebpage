...
/*
    all exports
*/
export { routeMeTo, getURLParams, fetchCurrentPageState, getPageName };
...
export default class RouterModule extends LightningElement {
    ...
    connectedCallback() {
        if (!this.registerPage) {
            //runs for c-router-module root component
            updatedURLCallback(this, this.resolveURL);
            this.resolveURL();
        }

        //below runs for c-router-module registrar components
        if (this.registerPage && this.isDefault && !getPageName()) {
            //to ensure page params are loaded on first page load
